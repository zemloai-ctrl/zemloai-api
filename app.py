...
logger = logging.getLogger("Zemlo-v1.9.5")
...
def compute_trust(ai_data):
    ...
    # FIX v1.9.5: dist_km=0 tarkoittaa epävarmaa dataa → lasketaan pisteitä
...
def get_signal():
    ...
    # FIX v1.9.5: Tyypitetyt poikkeukset weight-validoinnissa
    try:
    ...
    # 2. CACHE LAYER (5 min TTL)
    cache_key = f"z1.9.5:{hashlib.md5(f'{o_c}{d_c}{c_c}{int(weight)}'.encode()).hexdigest()}"
    try:
    ...
    # 3. LIVE INTELLIGENCE (Rinnakkain)
    ...
    try:
        ...
        # FIX v1.9.5: AI-vastauksen validointi palautettu (poistunut v1.9.4:ssä)
    ...
    # Parannettu hazardous-tunnistus: osittaiset osumat, monikot ja UN-numerot sanarajoilla
    ...
    response = {
        "signal": {
            ...
            # FIX v1.9.5: compute_trust() johdonmukaisesti, ei kovakoodattua inline-logiikkaa
        ...
        "metadata": {
            "engine":      "Zemlo v1.9.5",
            "request_id":  req_id[:8],
            # FIX v1.9.5: cache_hit mukana kaikissa vastauksissa
            "cache_hit":   False,
    ...
    # 7. STORAGE (Redis 5 min + Supabase)
    try:
...
def health():
    ...
    # FIX v1.9.5: Supabase-tarkistus lisätty
    try:
...
@app.route("/")
def index():
    return "Zemlo AI API v1.9.5 - Logistics Made Easy. Use /signal for queries."
